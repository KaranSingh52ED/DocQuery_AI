...
from src.embedding.embedding_models import (
...
def load_questions(questions_arg, questions_file_arg):
    ...
    What is the leave or time-off policy?
    How is performance reviewed?
    Are there any employee growth or learning programs?
...
def main():
    ...
    parser.add_argument(
        "--pdf_path", type=str, default=settings.PDF_PATH, help="Path to the PDF file."
    )
    parser.add_argument(
        "--llm_model",
        type=str,
    ...
    if embedding_model_name == "sentence-transformers":
        ...
        embedding_model = OpenAIEmbedding(
            settings.OPENAI_API_KEY, settings.OPENAI_EMBEDDING_MODEL_NAME
        )
    else:
    ...
    if llm_model_name == "gemini":
        ...
        if not settings.OPENAI_API_KEY:
            logger.error("OPENAI_API_KEY environment variable not set.")
            return
        llm_agent = OpenAILLMAgent(
            api_key=settings.OPENAI_API_KEY, model_name=settings.OPENAI_LLM_MODEL_NAME
        )
    else:
